...
class JointsDataset(Dataset):
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        if isinstance(cfg.MODEL.IMAGE_SIZE[0], (np.ndarray, list)):
            # print("joints_dataset image size clear")
            self.image_size = cfg.MODEL.IMAGE_SIZE
        else:
            self.image_size = cfg.MODEL.IMAGE_SIZE
            # self.image_size = [cfg.MODEL.IMAGE_SIZE] # real
        
        # self.heatmap_size = cfg.MODEL.HEATMAP_SIZE
        # print(f"HEATMAP SIZE TYPE => {type(cfg.MODEL.HEATMAP_SIZE[0])}")
        if isinstance(cfg.MODEL.HEATMAP_SIZE[0], (np.ndarray, list)):
            # print("joints_dataset multi heatmap size")
            self.heatmap_size = cfg.MODEL.HEATMAP_SIZE
        else:
            # print("joints_dataset single heatmap size")
            # self.heatmap_size = [cfg.MODEL.HEATMAP_SIZE] # real
            self.heatmap_size = cfg.MODEL.HEATMAP_SIZE
            # print(self.heatmap_size)
    ...
    def __getitem__(self, idx):
        ...
        input_img_mr = []
        joints_mr = []
        joints_single = joints
        if self.use_udp:
            # print(f"self.image_size => {self.image_size}")
        ...
        # input_img_mr = []
        # joints_mr = []
        # joints_single = copy.deepcopy(joints)
        # for i, imageSize in enumerate(self.image_size):
        #     if self.use_udp:
        #         trans = get_warp_matrix(rotation, center * 2.0, imageSize - 1.0, scale * 200.0)
        #         # print(f"[{i}] trans => {trans}\n\n")
        #         input_img = cv2.warpAffine(
        #             data_numpy,
        #             trans,
        ...
        #     "flip_pairs": self.flip_pairs,
        # }
        # # print(f"joints single 0 => {joints_mr[0]}")
        # # print(f"joints single 1 => {joints_mr[1]}")
        ...
        # if self.is_target_heatmap:
        #     heatmaps, heatmap_weight = self._generate_heatmap(joints, joints_vis)
    ...
    #     if self.heatmap_type == "gaussian":
    #         heatmaps = []
    #         for i, heatmapSize in enumerate(self.heatmap_size):
    ...
    #                 feat_stride = self.image_size[i] / heatmapSize
    ...
    #     return heatmaps, heatmap_weight
    ...
    def half_body_transform(self, joints, joints_visible):
        ...
        # aspect_ratio = self.image_size[0][0] / self.image_size[0][1] # real
