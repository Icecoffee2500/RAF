...
def main():
    ...
    args.world_size = ngpus_per_node * args.world_size # gpu 개수
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.wandb and gpu == 0:
        ...
        wdb.init(
            ...
            project="HPE_train_mpii_pure_vit",
            ...
            # name=f"Global_Token_added_{config.MODEL.NAME}_{config.MODEL.TYPE}_{config.DATASET.DATASET}_{config.LOSS.HM_LOSS}_{config.TRAIN.END_EPOCH}",
            name=f"Res_{config.MODEL.IMAGE_SIZE[1]}x{config.MODEL.IMAGE_SIZE[0]}_{config.MODEL.NAME}_{config.MODEL.TYPE}_{config.DATASET.DATASET}_{config.LOSS.HM_LOSS}_{config.TRAIN.END_EPOCH}",
        )
    ...
    print(f"ngpus_per_ndoe : {ngpus_per_node}")
    ...
    if "uncertainty" in args.cfg:
        ...
        if config.MODEL.KD_TARGET is not None:
            model = ViTPose(backbone, deconv_head, config.MODEL.KD_TARGET)
        else:
    ...
    valid_dataset = eval(f'dataset.{config.DATASET.DATASET}')(
        cfg=config,
        root=config.DATASET.ROOT,
    ...
    for epoch in range(config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH):
        lr_scheduler.step() # milestone에서 learning rate를 낮춰주게 하기 위해서 epoch을 세는 용도
